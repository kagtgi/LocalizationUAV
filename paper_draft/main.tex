...
\subsection{Qualitative Pipeline Examples}

Figures~\ref{fig:uav_results_01}--\ref{fig:uav_results_05} illustrate the
end-to-end pipeline on five representative UAV-VisLoc sites: (a) the satellite
reference map, (b) the preprocessed, yaw-aligned UAV query image, and (c) the
single-frame top-ranked candidate ($\star$) the descriptor matching returns.
These are illustrative of the pipeline's inputs and output, not of correct
localization: as quantified in Section~\ref{sec:analysis}, the single-frame
top candidate is typically far from the true position at full-map scale.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{0.48\linewidth}
        ...
        \caption{Top-ranked candidate (red $\star$).}
    \end{subfigure}
