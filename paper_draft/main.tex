...
paired with a stronger area-level observation or a learned embedding.
Our analysis provides the missing empirical account of why training-free
geometric matching, though attractive, does not by itself scale.
\end{abstract}
...
\item \textbf{A rigorous discriminability study on UAV-VisLoc.}
      ...
      supported in only $12\%$ of frames. We conclude that the limiting factor
      is the observation model, and identify what a scale-robust one must
      supply (Sections~\ref{sec:analysis}--\ref{sec:seq}).
\end{enumerate}
