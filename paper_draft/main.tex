...
standardization)---yet none lifts retrieval off $0\%$.
...
does not exploit.
Only when the search is constrained to a small area (a coarse prior of
${\approx}0.2$\,km$^{2}$, last row) does error fall to $148$\,m---but this is
the window mechanically bounding the error, not genuine identification: sub-$30$
m recall is $2.1\%$, barely above the ${\approx}1.4\%$ expected from a uniform
guess inside the window.
...
\midrule
\rowcolor{ours}
5-D + context, search $\le0.2$\,km$^2$ (coarse prior) & --- & 1.5\% & 148 \\
\bottomrule
\end{tabular}
\end{table}
...
It does not converge: the trajectory-level error stays at kilometre scale and
sub-$100$\,m recall is $0\%$.
The diagnostic explains why: the true position receives \emph{any} vote in only
$12\%$ of frames (median $0$ votes/frame), and when present its region ranks
$\approx\!123$rd. A particle filter integrates a signal that \emph{recurs};
ours is absent from $88\%$ of frames, so the filter instead locks onto a
spuriously consistent wrong track. The limiting factor is thus the
